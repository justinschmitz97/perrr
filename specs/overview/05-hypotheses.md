...
- 2026-04-28: round 4e.vii — **H9 closed ⇒ bug fixed.** Dual harness hooks for activeElement surfaced 235 silent divergences; root cause was two spec-compliance gaps in perrr-dom (default-to-body + disconnected-implicit-blur). Fixed. H10a (listener-counter mirror) also landed; H10b (full dispatch path) still open.
- 2026-04-28: round 4e.viii — H10b partially closed: listener *registry* (storage) implemented in perrr-dom (`Vec<Listener>` per node with id, event_type, capture, once, passive + spec-correct dedup). Dual harness migrated from counter to registry. Remaining for H10c: dispatch-path computation + JS-side firing of native-computed path + differential verification against HD's observed firing order.
