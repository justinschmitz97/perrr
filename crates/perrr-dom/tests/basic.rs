...
fn listener_counter_balances() {
    let mut tree = Tree::new();
    let el = tree.create_element("button");
    tree.add_event_listener(el, mk_listener(1, "click", false))
        .unwrap();
    tree.add_event_listener(el, mk_listener(2, "keydown", false))
        .unwrap();
...
fn add_event_listener_dedup_per_spec() {
    ...
    tree.add_event_listener(el, mk_listener(1, "click", true))
        .unwrap();
    assert_eq!(tree.listener_count(el), 2);
    // Different listener id on same type → new registration.
    tree.add_event_listener(el, mk_listener(2, "click", false))
        .unwrap();
